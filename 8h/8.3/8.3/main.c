...
{	
	...
	"{\"name\":\"team\",\"value\":\"%s\"},"
	"{\"name\":\"status\",\"value\":\"%s\"}]"
	,print_celsious, print_ADC, "40", status);
	usart_transmit_to_string("ESP:payload:\"");
	usart_transmit_to_string(payload);
	usart_transmit_to_string("\"\n");
	_delay_ms(1000);
	lcd_clear_display();
	memset(response, 0, sizeof(response));
	usart_receive_string(response);
	if(!strcmp(response,"\"Success\"")){
		lcd_write_string("3.Success");
	}else{
		lcd_write_string("3.FAIL");
		continue;
	}
	_delay_ms(1000);
	lcd_clear_display();
	memset(response, 0, sizeof(response));
	usart_transmit_to_string("ESP:transmit\n");
	usart_receive_string(response);
	lcd_write_string(response);
	_delay_ms(1000);
	lcd_clear_display();
